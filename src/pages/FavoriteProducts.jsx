...
const FavoriteProducts = () => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        const res = await axios.get(`https://my-json-server.onrender.com/likeproduct?user_name=${userName}`);
        ...
        if (res.data.length > 0) {
          ...
          if (userFavorites.length > 0) {
            ...
            const productPromises = ids.map(id => axios.get(`https://my-json-server.onrender.com/products/${id}`));
            try {
  ...
  const handleRemove = async (productId) => {
    ...
    if (like) {
      try {
        await axios.delete(`https://my-json-server.onrender.com/likeproduct/${like.id}`);
